...
class UsersController < ApplicationController
  ...
  def eliminar_deseado
    ...
    producto = Product.find(deseado)

    # If deseado is in the list of deseados, save boolean in variable
    deseado_in = current_user.deseados.include?(producto)
    current_user.deseados.delete(producto)

    if deseado_in && current_user.save!(validate: false)
