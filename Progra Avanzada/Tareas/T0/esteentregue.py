...
def imprimir_menu(usuario):
    ...
    if respuesta == 3: 
        ...
        menu_publicaciones("publicaciones.csv",usuario) 
    elif respuesta == 2:
...
def menu_publicaciones(path,user):
    usuario = user
    ...
    respuesta = int(input("Indique su opción:"))
    if respuesta != cont:
        id_a_buscar = listafinal[respuesta][2]
    if respuesta != cont: 
    ...
    if respuesta == cont:
        ...
        desplegar_info(lista_publicaciones,buscar,"comentarios.csv",usuario,id_a_buscar)
...
def desplegar_info(lista,encontrar,path,user,id):
    id_a_buscar = id
    usuario = user
    for i in lista:
        if i[1] == encontrar:
    ...
    if respuesta == 1:
        agregar_comentario(path,usuario,id_a_buscar)
    elif respuesta == 2:
        menu_publicaciones("publicaciones.csv",usuario)
    else: 
...
def agregar_comentario(path,user,id):
    usuario = user.strip()
    id_a_buscar = id
    fecha = str(datetime.datetime.now())
    comentario = input("Ingrese su comentario:")
    with open(path,"a") as myfile:
        myfile.write("\n"+id+","+usuario+","+fecha+"," +comentario)
