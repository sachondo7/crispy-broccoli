...
def filtrar_prohibidos(iterar_peliculas, actor_prohibido):
    ...
    return filter(lambda pelicula: actor_prohibido != pelicula.actores , iterar_peliculas)
...
def calcular_afinidades(catalogo_peliculas, usuario: Usuario):
    ...
    """
    # Debes completar esta función
    return map(lambda x: (usuario.calcular_afinidad, catalogo_peliculas), catalogo_peliculas)
...
def encontrar_peliculas_comunes(usuarios_watch_party):
    ...
    """
    # Debes completar esta función
    intersección = reduce(lambda x, y: x & y, usuarios_watch_party)
    return set(intersección)
...
def encontrar_usuario_mas_afin(usuario, otros_usuarios):
    ...
    """
    conjunto = filter(lambda pelicula: usuario.actor_prohibido == otros_usuarios.actor_prohibido,otros_usuarios)
    otro = reduce(lambda x,y: x if x+ usuario > y + usuario else y,conjunto )
    # Debes completar esta función
